...
<div class="app-main__outer">
    <div class="app-main__inner">
       ...
       <div class="container">
                <form class="form-horizontal" method="POST" action="{{ route('hr.store_edit_team') }}" onsubmit="return validateForm();">
                    {{ csrf_field() }}
                    <div class="form-row" id="team_form">
                        <div class="col-md-6">
                            <div class="position-relative form-group{{ $errors->has('name') ? ' has-error' : '' }}">
                                ...
                                <input name="name" value="{{ $team->name }}"  id="exampleName" placeholder="Name here..." type="text" class="form-control" >
                                <input type="hidden" name="team_id" value="{{ $team->id }}"  id="team_id"  class="form-control" >
                                @if ($errors->has('name'))
                        ...
                        <div class="col-md-6 ">
                            <br>
                            <button id="submit" type="submit" name="submit" class="mt-2 btn btn-lg btn-primary">Update Team</button>
                        </div>
                    </div>   
                </form>   
                            ...
                            <div class="col-12 col-sm-8 col-md-6 col-lg-3" style="margin-bottom:12%;"  id="uid{{$usr->id}}">
                                <div class="card" style="height:100%">
                                    <div class="card-body text-center">
                                        ...
                                        <input type="checkbox" name="{{'sel'.$usr->id}}"
                                            @if(in_array($usr->id,$sel_users))
                                                checked="checked" 
                                            @endif 
                                         onclick="append_sel(this.value)" value="{{$usr->id}}">
                                    </div>
                                </div>
                            </div>
                        @endif
